...
public class DialogChangeScreenName extends Dialog {
    Context mContext;
    ...
    public DialogChangeScreenName(Context mContext, String screenName, ScreenNameChangeDialogResponse dialogResponseListener) {
        super(mContext);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        CommonUtils.hideTextSuggestion(txtScreenName);
    }
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.btn_save:
                if (validate()) {
                    if (txtScreenName.getText().toString().trim().length() < 21) {
                        dialogResponseListener.onChangeName(txtScreenName.getText().toString().trim());
                        dismiss();
                    } else {
                        Toast.makeText(mContext, mContext.getString(R.string.screen_name_should_be_less_then_20_characters), Toast.LENGTH_SHORT).show();
                    }
                }
                break;
            case R.id.txtcancel:
