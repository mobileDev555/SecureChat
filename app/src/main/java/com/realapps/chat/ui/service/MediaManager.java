...
import com.realapps.chat.ui.utils.Ringer;
import com.realapps.chat.ui.utils.accessibility.AccessibilityWrapper;
import com.realapps.chat.ui.utils.audio.AudioFocusWrapper;
import com.realapps.chat.ui.utils.bluetooth.BluetoothWrapper;
import com.realapps.chat.ui.utils.bluetooth.BluetoothWrapper.BluetoothChangeListener;
...
public class MediaManager implements BluetoothChangeListener {
	...
	public MediaManager(SipService aService) {
		...
		System.out.println("InCall Activity : Media maanger "+supportMultipleCalls);
		/*if(supportMultipleCalls){
			mediaStateChangedIntent.putExtra(SipManager.EXTRA_CONFERENCE_CALL_SETUP, "true");
		}else{
			mediaStateChangedIntent.putExtra(SipManager.EXTRA_CONFERENCE_CALL_SETUP, "false");
		}
		*/
    ...
    private synchronized void actualSetAudioInCall() {
		...
		*/
		...
		//Acquire wifi lock
		@SuppressLint("WifiManagerLeak") WifiManager wman = (WifiManager) service.getSystemService(Context.WIFI_SERVICE);
		if(wifiLock == null) {
		...
		if(winfo != null) {
			...
			if(service.getPrefs().getPreferenceBooleanValue(SipConfigManager.KEEP_AWAKE_IN_CALL)) {
				if(screenLock == null) {
		            ...
		            screenLock = pm.newWakeLock(PowerManager.SCREEN_DIM_WAKE_LOCK | PowerManager.ON_AFTER_RELEASE, "com.csip.onIncomingCall.SCREEN");
...
//		audioManager.setRingerMode(prefs.getInt("savedRingerMode", AudioManager.RINGER_MODE_NORMAL));
	...
	synchronized public void startRing(String remoteContact) {
		...
		if(!ringer.isRinging()) {
			...
			Log.e(THIS_FILE, "Already ringing ....");
		}
