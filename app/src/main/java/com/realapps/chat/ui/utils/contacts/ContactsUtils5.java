...
package com.realapps.chat.ui.utils.contacts;
...
public class ContactsUtils5 extends ContactsWrapper {
    ...
    public Cursor getContactsPhones(Context ctxt, CharSequence constraint) {
        ...
        String query= Contacts.HAS_PHONE_NUMBER + " > '0'"+" AND "+ Contacts.DISPLAY_NAME +" LIKE '%"+constraint+"%'";
        // String query=ContactsContract.Contacts.HAS_PHONE_NUMBER + " > '0'";
        System.out.println("step query :"+query);
        Cursor resCursor = ctxt.getContentResolver().query(uri,
                projection, query, null, Contacts.DISPLAY_NAME + " COLLATE LOCALIZED ASC");
        return resCursor;
    }
    ...
    public CharSequence transformToSipUri(Context ctxt, Cursor cursor) {
    	...
    	Cursor pCur =  ctxt.getContentResolver().query(
                ...
                ContactsContract.CommonDataKinds.Phone.CONTACT_ID + " = " + contactId + " AND " + CommonDataKinds.Phone.TYPE + " = " + CommonDataKinds.Phone.TYPE_MOBILE, null,
                ContactsContract.CommonDataKinds.Phone._ID + " desc");*/


       /* if( pCur.moveToFirst()){
            do {
                phoneNo = pCur.getString(pCur.getColumnIndex(CommonDataKinds.Phone.NUMBER));
            }while(pCur.moveToNext());
        }*/
    	try{
    		 value = cursor.getString(cursor.getColumnIndex(CommonDataKinds.Phone.NUMBER));
    	        if (value == null) {
    	            return "";
    	        }
    	}catch(Exception e){
    ...
    public boolean isExternalPhoneNumber(Context context, Cursor cursor) {
        String mimeType = cursor.getString(cursor.getColumnIndexOrThrow(CommonDataKinds.Phone.MIMETYPE));
    ...
    public void bindContactPhoneView(View view, Context context, Cursor cursor) {
        ...
        /*String value = cursor.getString(cursor.getColumnIndexOrThrow(CommonDataKinds.Phone.NUMBER));*/
        String displayName = cursor.getString(cursor.getColumnIndexOrThrow(Contacts.DISPLAY_NAME));
        Long contactId = cursor.getLong(cursor.getColumnIndexOrThrow(Contacts._ID));

        Cursor pCur =  context.getContentResolver().query(
                CommonDataKinds.Phone.CONTENT_URI,
                new String[]{CommonDataKinds.Phone._ID,
                        CommonDataKinds.Phone.CONTACT_ID,
                        CommonDataKinds.Phone.NUMBER},
                      //  ContactsContract.CommonDataKinds.Phone.MIMETYPE},
                CommonDataKinds.Phone.CONTACT_ID + " = " + contactId + " AND " + CommonDataKinds.Phone.TYPE + " = " + CommonDataKinds.Phone.TYPE_MOBILE, null,
                CommonDataKinds.Phone._ID + " desc");


        if( pCur.moveToFirst()){
            do {
                phoneNo = pCur.getString(pCur.getColumnIndex(CommonDataKinds.Phone.NUMBER));
              /*  String mimeType = pCur.getString(pCur.getColumnIndex(ContactsContract.CommonDataKinds.Phone.MIMETYPE));//Data.MIMETYPE));
                if(CommonDataKinds.Phone.CONTENT_ITEM_TYPE.equalsIgnoreCase(mimeType)) {
                    int typeColumnIdx = pCur.getColumnIndex(CommonDataKinds.Phone.TYPE);
                    int labelColumnIdx = pCur.getColumnIndex(CommonDataKinds.Phone.LABEL);
                    if(labelColumnIdx != -1 && typeColumnIdx != -1) {
                        String labelField = pCur.getString(labelColumnIdx);
                        int typeField = pCur.getInt(typeColumnIdx);
                    ...
                    int typeColumnIdx = pCur.getColumnIndex(CommonDataKinds.Im.PROTOCOL);
                    int labelColumnIdx = pCur.getColumnIndex(CommonDataKinds.Im.CUSTOM_PROTOCOL);
        ...
        Bitmap bitmap = getContactPhoto(context, uri, false, null);
        //CharSequence labelName = "";
        ...
        //System.out.println("Contact Name:" + displayName + " & Number: " + phoneNo );
        ...
        if(phoneNo.length() != 0){
            ...
            if(imageView != null) {
                if(bitmap == null){
                    ...
                    String letter = gc.split_word(displayName);
    ...
    public void updateCSipPresence(Context ctxt, String buddyUri,
        ...
        if (Compatibility.isCompatible(8)) {
            ...
            builder.withValue(StatusUpdates.STATUS_ICON, R.mipmap.ic_launcher);
