...
package com.realapps.chat.ui.pjsip;
...
import com.realapps.chat.ui.api.SipUri.ParsedSipContactInfos;
import com.realapps.chat.ui.helper.PrefManager;
import com.realapps.chat.ui.service.MediaManager;
import com.realapps.chat.ui.service.SipNotifications;
import com.realapps.chat.ui.service.SipService;
import com.realapps.chat.ui.service.SipService.SameThreadException;
import com.realapps.chat.ui.service.SipService.SipRunnable;
import com.realapps.chat.ui.service.impl.SipCallSessionImpl;
import com.realapps.chat.ui.service.receiver.ConferenceCallerIdReceiver;
import com.realapps.chat.ui.utils.CallLogHelper;
import com.realapps.chat.ui.utils.Compatibility;
import com.realapps.chat.ui.utils.Log;
import com.realapps.chat.ui.utils.Threading;
import com.realapps.chat.ui.utils.TimerWrapper;
...
public class UAStateReceiver extends Callback {//implements ConferenceCallerIdReceiver.IConferenceCallerID {
    ...
    private static boolean supportMultipleCall;

    private static ConferenceCallerIdReceiver.IConferenceCallerID iConferenceCallerID;
    ...
    /*@Override
    public void sendCallerID(ArrayList<Integer> sendCallerIdArray) {
            System.out.println(THIS_FILE + " sendCallerID " + sendCallerIdArray);
    }*/
    ...
    public void on_incoming_call(final int accId, final int callId, SWIGTYPE_p_pjsip_rx_data rdata) {
        lockCpu();
        /*iConferenceCallerID = this;
        ConferenceCallerIdReceiver conferenceCallerIdReceiver = new ConferenceCallerIdReceiver();
        conferenceCallerIdReceiver.registerCallback(iConferenceCallerID);*/
        ...
        try {
            ...
            if (remContact.contains("<") && remContact.contains("@") && remContact.contains(":"))
                remContact = remContact.substring(remContact.indexOf(":") + 1, remContact.indexOf("@"));
            callInfo.setIncoming(true);
            String name = CommonUtils.getContactName(ctx, remContact);

            // need to remove because call Activity not showing
            /*if(remContact.equalsIgnoreCase(name)) {
               return;
            }*/
    ...
    public void on_call_state(final int callId, pjsip_event e) {
        ...
        try {
            ...
            if (callState == SipCallSession.InvState.DISCONNECTED) {
                ...
                System.out.println("History fetchert::: UAstateReceiver flag 1 ::: " + prefManager.getHistoryFetcher_AfterCall());
                //prefManager.setHistoryFetcher_AfterCall(true);
                System.out.println("History fetchert::: UAstateReceiver flag 2 ::: " + prefManager.getHistoryFetcher_AfterCall());
...
//        notificationManager.showNotificationForMessage(msg);
    ...
    public void on_call_media_state(final int callId) {
        ...
        try {
            ...
            System.out.println(THIS_FILE + " on_call_media_state call << for account " + callId);
...
//                            }
    ...
    private SipCallSessionImpl updateCallInfoFromStack(Integer callId, pjsip_event e)
        ...
        System.out.println(THIS_FILE + " Updating call infos from the stack ");
...
//                            stateReceiver.notificationManager.showNotificationForCall(callInfo);
                        ...
                        case SipCallSession.InvState.DISCONNECTED:
                            ...
                            //======== when end the calling, sound mode : normal
                            AudioManager audioManager = (AudioManager) SipService.context.getSystemService(Context.AUDIO_SERVICE);
                            ...
                            if (callInfo.getLastStatusCode() != 200) {
                                ...
                                if (callInfo.getLastStatusCode() == 603) {
                                    stateReceiver.pjService.service.notifyUserOfMessage("SS Audio declined");
                                } else if (callInfo.getLastStatusCode() == 604) {

                                } else if (callInfo.getLastStatusCode() == 480) {

                                } else if (callInfo.getLastStatusCode() == 486) {

                                } else if (callInfo.getLastStatusCode() == 487) {
                                    System.out.println("Test Call status code: " + callInfo.getLastReasonCode() + ":" + callInfo.getLastStatusCode());
                                    stateReceiver.pjService.service.notifyUserOfMessage("SS Audio Ended");
                                } else {
...
/*    private BroadcastReceiver mConferenceCallerId = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if(intent.getAction().equals(SipManager.ACTION_FOR_CONFERENCE_CALLERID)){
                ...
                System.out.println(THIS_FILE + " action "+ intent.getAction());
                System.out.println(THIS_FILE + " caller_id_list "+ caller_id_list.size());
            }
        }
    };*/
    ...
    public void initService(PjSipService srv) {
        ...
        supportMultipleCall = pjService.service.supportMultipleCalls;
    ...
    /*public void activeAutoRecord(Context ctxt){
    	mAutoRecordCalls = SipConfigManager.getPreferenceBooleanValue(ctxt,
                SipConfigManager.AUTO_RECORD_CALLS);
    }*/
    ...
    private void onBroadcastCallState(final SipCallSession callInfo) {
        ...
        //Check support multiple call is allowed or not a-nd based on that handle dual screen lunching code
        if (callInfo.getCallState() == 6) {
            callStateChangedIntent.putExtra(SipManager.EXTRA_CONFERENCE_CALL_SETUP, "false");
        }
