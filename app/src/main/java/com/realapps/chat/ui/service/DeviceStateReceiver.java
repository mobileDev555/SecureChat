...
import com.realapps.chat.ui.utils.PhoneCapabilityTester;
...
public class DeviceStateReceiver extends BroadcastReceiver {
    ...
    public static final String APPLY_NIGHTLY_UPLOAD = "com.csip.action.APPLY_NIGHTLY";
    ...
    private boolean checkPermission(Context ctx){
        int read_contact = ContextCompat.checkSelfPermission(ctx, Manifest.permission.READ_CONTACTS);
        int write_contact = ContextCompat.checkSelfPermission(ctx, Manifest.permission.WRITE_CONTACTS);
        int read_phone = ContextCompat.checkSelfPermission(ctx, Manifest.permission.READ_PHONE_STATE);
        int record_audio = ContextCompat.checkSelfPermission(ctx, Manifest.permission.RECORD_AUDIO);
        int modify_audio = ContextCompat.checkSelfPermission(ctx, Manifest.permission.MODIFY_AUDIO_SETTINGS);
        int read_storage = ContextCompat.checkSelfPermission(ctx, Manifest.permission.READ_EXTERNAL_STORAGE);
        int write_storage = ContextCompat.checkSelfPermission(ctx, Manifest.permission.WRITE_EXTERNAL_STORAGE);
        int config_sip = ContextCompat.checkSelfPermission(ctx, SipManager.PERMISSION_CONFIGURE_SIP);


        if (read_contact == PackageManager.PERMISSION_GRANTED && write_contact == PackageManager.PERMISSION_GRANTED && read_phone == PackageManager.PERMISSION_GRANTED && record_audio ==  PackageManager.PERMISSION_GRANTED && modify_audio ==  PackageManager.PERMISSION_GRANTED && read_storage == PackageManager.PERMISSION_GRANTED && write_storage == PackageManager.PERMISSION_GRANTED && config_sip == PackageManager.PERMISSION_GRANTED){
            return true;
        } else {
            return false;
        }
