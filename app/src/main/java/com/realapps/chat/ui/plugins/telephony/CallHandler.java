 ...
 */
package com.realapps.chat.ui.plugins.telephony;
...
public class CallHandler extends BroadcastReceiver {
	...
	// code from Asfak
	private DBAdapter.DatabaseHelper dbHelp;
	...
	public void onReceive(Context context, Intent intent) {
		if(SipManager.ACTION_GET_PHONE_HANDLERS.equals(intent.getAction())) {
			...
			if(number != null && PhoneCapabilityTester.isPhone(context)) {
				...
				/*// to insert the value to the database for incoming and out going call
				dbHelp = new DatabaseHelper(context);
				dbHelp.insertcalllog(number, "incoming", ctime, duration);*/
			}
			...
			//Boast.makeText(context, ""+number, Toast.LENGTH_LONG).show();
